...
export const setUsername = async (store: Store, password: string, userName: string): Promise<React.Component> => {
  const walletDom = await signUp(store, password);
  const inputs = TestUtils.scryRenderedDOMComponentsWithTag(walletDom, "input");
  const username = inputs[0];
  TestUtils.Simulate.change(username, { target: { value: userName } } as any);
...
export const travelToBalance = async (store: Store, password: string): Promise<React.Component> => {
...
xdescribe("DOM > Feature > Balance", () => {
  ...
  beforeEach(async (done) => {
    ...
    store = aNewStore();
    walletDom = await setUsername(store, profilePass, profileUsername);
    done();
  });
  ...
  mayTestBns(`should redirect to ${SET_NAME_ROUTE} route after success login`, async (done) => {
    const inputs = TestUtils.scryRenderedDOMComponentsWithTag(walletDom, "input");
