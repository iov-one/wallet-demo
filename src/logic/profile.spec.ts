...
describe("getWalletAndIdentity", () => {
  it("should error if no keyrings", async () => {
    ...
    expect(() => getWalletAndIdentity(profile, "bns-chain" as ChainId)).toThrowError(/No identity found/);
  });
  ...
  it("should return identity if this chain registered - ed25519", async () => {
    ...
    const { walletId, identity } = getWalletAndIdentity(profile, "bns-chain" as ChainId);
